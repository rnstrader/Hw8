...
get_cv_splits <- function(data, num_folds) {
  ...
  folds <- split(data[random_indices, ], rep(1:num_folds, each = size_fold, length.out = nrow(data)))
  
  cv_splits <- vector("list", num_folds)
  
  for (i in seq_len(num_folds)) {
    test_data  <- folds[[i]]
    train_data <- do.call(rbind, folds[-i])
    cv_splits[[i]] <- list(train = train_data, test = test_data)
  }
...
cv_splits <- get_cv_splits(bike_train, 10)
```
...
bike_rec <- recipe(total_bike_count ~ ., data = bike_train) |> 
  step_date(date, features = "wday", keep_original_cols = FALSE) |>
  step_mutate(
    day_type = factor(
      if_else(date_wday %in% c("Sat", "Sun"), "weekend", "weekday")
    )
  ) |>
  step_rm(date_wday, date) |> 
